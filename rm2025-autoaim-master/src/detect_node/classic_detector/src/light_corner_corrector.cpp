...
void LightCornerCorrector::correctCorners(Armor& armor, const cv::Mat& gray_img) {
    ...
    if (armor.left_light.width > PASS_OPTIMIZE_WIDTH) {
        ...
        // fmt::print("fsa time: {}ms\n", std::chrono::duration_cast<std::chrono::microseconds>(e - s).count() / 1000.);
...
SymmetryAxis LightCornerCorrector::findSymmetryAxis(const cv::Mat& gray_img, const Light& light) {
    ...
    cv::Point2f centroid =
        cv::Point2f(moments.m10 / moments.m00, moments.m01 / moments.m00) + cv::Point2f(light_box.x, light_box.y);
    ...
    // tbb::concurrent_vector<cv::Point2f> cpoints;
    // tbb::parallel_for(0, roi.rows, [&cpoints, &roi](int i) {
    //     tbb::parallel_for(0, roi.cols, [&cpoints, &roi, &i](int j) {
    //         tbb::parallel_for(0, static_cast<int>(std::round(roi.at<float>(i, j))), [&cpoints, &i, &j](int) {
    //             cpoints.emplace_back(j, i);
    //             ;
    //         });
    //     });
    // });
    // std::vector<cv::Point2f> points{cpoints.begin(), cpoints.end()};

    cv::Mat points_mat = cv::Mat(points).reshape(1);
    // auto e = std::chrono::steady_clock::now();
    // fmt::print("PCA time: {}ms\n",
    //            std::chrono::duration_cast<std::chrono::microseconds>(e - s).count() / 1000.);
