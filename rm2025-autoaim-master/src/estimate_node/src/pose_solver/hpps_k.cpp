//
// Created by root on 7/19/24.
//

#include "estimate_node/pose_solver/hpps_k.h"
using namespace FP;

std::optional<double> HPPSK::Run(const cv::Point3d& armor_pose,
                                 const double input_imu_yaw,
                                 const double input_imu_pitch,
                                 const double input_imu_roll,
                                 const double input_yaw_motor,
                                 const double input_pitch_motor,
    ...
    return Trichotomy(input_imu_yaw, input_imu_pitch, input_imu_roll, input_yaw_motor, input_pitch_motor, armor_pose,
                      armor_size, image_armor_points, estimated_armor_yaw, is_outpost_spin, use_sencondary_camera,
                      inclined);
}

[[maybe_unused]] double HPPSK::ComputeIOU(const HPPSK::Contour& cont_1,
                                          const HPPSK::Contour& cont_2,
                                          const cv::Size& roi) {
...
std::vector<cv::Point2f> HPPSK::Reprojection(const double armor_yaw,
                                             const double imu_yaw,
                                             const double imu_pitch,
                                             const double imu_roll,
                                             const double yaw_motor,
                                             const double pitch_motor,
                                             const cv::Point3d& t,
                                             const RM::ArmorSize armor_size,
                                             const bool is_outpost_spin,
    ...
    R_camera_to_pitch = R_camera_to_pitch_install_yaw_offset * R_camera_to_pitch_install_pitch_offset *
                        R_camera_to_pitch;
    ...
    cv::Mat T_camera_to_geographic = T_chassis_to_geographic * T_camera_to_chassis;
    ...
    std::for_each(armor_point_vec.begin(), armor_point_vec.end(),
                  [&T_model_to_chassis, &T_camera_to_geographic](cv::Point3f& p) {
                      cv::Mat point_mat = (cv::Mat_<double>(4, 1) << p.x, p.y, p.z,
                                           1.0);  // �?�?为齐次左边坐标进行坐标转�?

                      //                      std::cout << t << std::endl;

                      point_mat = T_model_to_chassis * point_mat;

                      //                      std::cout << T_model_to_chassis << std::endl;
...
std::optional<double> HPPSK::Trichotomy(double imu_yaw,
                                        double imu_pitch,
                                        double imu_roll,
                                        const double yaw_motor,
                                        const double pitch_motor,
    ...
    double left = angles::normalize_angle(estimated_armor_yaw + param_.trichotomy_min_offset);
    double right = angles::normalize_angle(estimated_armor_yaw + param_.trichotomy_max_offset);
    ...
    while (count <= param_.trichotomy_max_iterations) {
        ...
        auto re_mid1 = Reprojection(mid_1, imu_yaw, imu_pitch, imu_roll, yaw_motor, pitch_motor, t, armor_size,
                                    is_outpost_spin, use_sencondary_camera);
        auto re_mid2 = Reprojection(mid_2, imu_yaw, imu_pitch, imu_roll, yaw_motor, pitch_motor, t, armor_size,
                                    is_outpost_spin, use_sencondary_camera);
...
double HPPSK::SJTU_get_pts_cost(const std::vector<cv::Point2f>& cv_refs,
